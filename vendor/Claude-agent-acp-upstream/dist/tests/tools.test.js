...
describe("empty message content is not emitted", () => {
    ...
    it("drops an empty string content instead of emitting an empty agent_message_chunk", () => {
        const notifications = toAcpNotifications("", "assistant", "test-session", {}, mockClient, mockLogger);
        expect(notifications).toHaveLength(0);
    });
    it("still emits non-empty string content", () => {
        const notifications = toAcpNotifications("hello", "assistant", "test-session", {}, mockClient, mockLogger);
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
    ...
    it("drops an empty streamed text block but keeps a non-empty one", () => {
        const notifications = toAcpNotifications([
            { type: "text", text: "" },
            { type: "text", text: "real" },
        ], "assistant", "test-session", {}, mockClient, mockLogger);
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "agent_message_chunk",
...
describe("Agent/Task tool_result rendering from tool_use_result", () => {
    ...
    it("threads options.toolUseResult through toAcpNotifications for a lone tool_result", () => {
        const toolUseCache = { toolu_agent: agentToolUse };
        const notifications = toAcpNotifications([rawResult], "user", "test-session", toolUseCache, mockClient, mockLogger, { toolUseResult: structured });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "tool_call_update",
    ...
    it("ignores options.toolUseResult when several tool_result blocks are batched", () => {
        ...
        const notifications = toAcpNotifications([rawResult, { ...rawResult, tool_use_id: "toolu_agent2" }], "user", "test-session", toolUseCache, mockClient, mockLogger, { toolUseResult: structured });
        expect(notifications).toHaveLength(2);
...
describe("structured tool_use_result rendering (Read/Bash/WebSearch)", () => {
    describe("Read", () => {
        ...
        it("defaults startLine to 1 when both startLine and offset are absent", () => {
            const update = toolUpdateFromToolResult(rawWithReminder, { ...readToolUse, input: { file_path: "/tmp/f.ts" } }, false, {
                type: "text",
                file: { filePath: "/tmp/f.ts", content: "one\ntwo" },
            });
            expect(update.content?.[0]).toEqual({
        ...
        it("falls back to raw content for non-text variants", () => {
            ...
            expect((update.content?.[0]).content.text).toContain("const a = 1;");
        });
    });
    describe("Bash", () => {
        const bashToolUse = {
    ...
    describe("WebSearch", () => {
        ...
        it("falls back to the raw dump when tool_use_result is absent", () => {
            const update = toolUpdateFromToolResult(rawDump, searchToolUse, false);
            expect((update.content?.[0]).content.text).toContain("Web search results for query");
        });
