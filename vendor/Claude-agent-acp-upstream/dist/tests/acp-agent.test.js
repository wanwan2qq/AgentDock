...
function lifecycleFrame(commandUuid, state) {
    return {
...
describe("tool conversions", () => {
    ...
    it("marks Agent and legacy Task tool calls as subagent launches", () => {
        ...
        ], "assistant", "test-session", {}, {}, console, { parentToolUseId: "outer-agent" });
...
describe("synthetic login message (issue #863)", () => {
    ...
    it("isSyntheticLoginMessage matches the CLI auth-error message", () => {
        ...
        expect(isSyntheticLoginMessage({
    ...
    it("loadSession replay skips the synthetic login message but keeps the rest", async () => {
        ...
        expect(updates.some((u) => u.update.sessionUpdate === "user_message_chunk" &&
            u.update.content.type === "text" &&
            u.update.content.text.includes("hi, say one word"))).toBe(true);
...
describe("tool_call emitted before permission request", () => {
    ...
    it("emits the tool_call (then asks permission) when the stream hasn't yet", async () => {
        ...
        const result = await agent.canUseTool("session-1")("Bash", { command: "git diff", description: "Show current diff" }, {
            signal: new AbortController().signal,
            suggestions: [],
            toolUseID: "tool-1",
        });
    ...
    it("resolves a permission-surfaced TodoWrite tool_call at tool_result time", () => {
        ...
        const notifications = toAcpNotifications([{ type: "tool_result", tool_use_id: "todo-1", content: [{ type: "text", text: "ok" }] }], "user", "session-1", session.toolUseCache, {}, console, { emittedToolCalls: session.emittedToolCalls });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
    ...
    it("marks a permission-surfaced TodoWrite tool_call failed on an is_error tool_result", () => {
        ...
        ], "user", "session-1", session.toolUseCache, {}, console, { emittedToolCalls: session.emittedToolCalls });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "tool_call_update",
            toolCallId: "todo-1",
    ...
    it("resolves permission-surfaced Task* tool_calls too", () => {
        ...
        const notifications = toAcpNotifications([{ type: "tool_result", tool_use_id: "task-1", content: [{ type: "text", text: "[]" }] }], "user", "session-1", session.toolUseCache, {}, console, { emittedToolCalls: session.emittedToolCalls });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "tool_call_update",
            toolCallId: "task-1",
...
describe("subagent permission attribution (issue #851)", () => {
    ...
    it("restores the attribution via the streamed refinement when the eager emission raced task_started", () => {
        ...
        const notifications = toAcpNotifications([{ type: "tool_use", id: "toolu_sub", name: "Bash", input: { command: "ls" } }], "assistant", "session-1", session.toolUseCache, {}, console, { emittedToolCalls: session.emittedToolCalls, parentToolUseId: "toolu_parent" });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "tool_call_update",
            toolCallId: "toolu_sub",
    ...
    function makeGenerator(messages) {
    ...
    it("records task_started's task_id → tool_use_id mapping while consuming the stream", async () => {
        const agent = new ClaudeAcpAgent({ sessionUpdate: vi.fn(async () => { }) }, {
            log: () => { },
            error: () => { },
        });
        injectGeneratorSession(agent, makeGenerator([taskStarted("agent-42", "toolu_parent"), successResult()]));
        await agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "go" }] });
        expect(agent.sessions["test-session"].liveBackgroundTasks.get("agent-42")?.parentToolUseId).toBe("toolu_parent");
    });
...
describe("stop reason propagation", () => {
    ...
    it("only reconciles Fast mode from user-driven results, not task-notification followups", async () => {
        ...
        const agent = new ClaudeAcpAgent(mockClient, { log: () => { }, error: () => { } });
        agent.clientCapabilities = { session: { configOptions: { boolean: {} } } };
        const input = new Pushable();
        ...
        expect(agent.sessions["test-session"].fastModeEnabled).toBe(true);
        const configUpdates = updates.filter((n) => n.update?.sessionUpdate === "config_option_update");
        expect(configUpdates).toHaveLength(1);
        expect(configUpdates[0].update.configOptions).toContainEqual(createFastModeConfigOption(true, true));
    });
    ...
    it("publishes active_goal as provider-neutral session state without changing turn settlement", async () => {
        ...
        await expect(agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "test" }] })).resolves.toEqual(expect.objectContaining({ stopReason: "end_turn" }));
        ...
        expect(updates.some((notification) => notification.update?._meta?.claudeCode?.goal)).toBe(false);
    });
    ...
    it("rolls back an optimistic goal update when the goal command fails", async () => {
        ...
        expect(goalUpdates.at(-2)).toEqual(expect.objectContaining({ objective: "Replacement", status: "active" }));
        expect(goalUpdates.at(-1)).toEqual(expect.objectContaining({ objective: "Original", iterations: 2 }));
    });
...
describe("model refusal fallback handling", () => {
    ...
    function refusalFallbackMessage(overrides = {}) {
        return {
            type: "system",
    ...
    function createCapturingAgent() {
        ...
        const agent = new ClaudeAcpAgent({ sessionUpdate }, {
            log: () => { },
            error: () => { },
        });
    ...
    it("notifies the user and reconciles model state on model_refusal_fallback", async () => {
        ...
        injectGeneratorSession(agent, makeGenerator([refusalFallbackMessage(), successResult()]), modelStateOverrides);
        const response = await agent.prompt({
            sessionId: "test-session",
            prompt: [{ type: "text", text: "test" }],
        });
        ...
        const updates = sessionUpdate.mock.calls.map((c) => c[0].update);
        const notice = updates.find((u) => u.sessionUpdate === "agent_message_chunk" && u.content.text.includes("Model fallback"));
        ...
        const modelOption = configUpdate.configOptions.find((o) => o.id === "model");
    ...
    it("does not persist the swap for a turn-only fallback (direction revert)", async () => {
        const { agent, sessionUpdate } = createCapturingAgent();
        injectGeneratorSession(agent, makeGenerator([refusalFallbackMessage({ direction: "revert" }), successResult()]), modelStateOverrides);
        await agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "test" }] });
        const session = agent.sessions["test-session"];
    ...
    it("ignores a non-human-authored refusal banner (refused_user_message_uuid null)", async () => {
        ...
        // user's turn.
        expect(updates.some((u) => u.sessionUpdate === "agent_message_chunk" &&
            u.content.text.includes("Background task declined."))).toBe(false);
    });
});
describe("logout", () => {
    function createMockAgent() {
...
describe("assembled assistant text fallback", () => {
    ...
    it("forwards subagent text and thinking as nested chunks for capable clients", async () => {
        ...
        await agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "hi" }] });
        const nestedUpdates = updates.filter(({ update }) => update.sessionUpdate === "agent_message_chunk" ||
    ...
    function injectSessionTwoTurns(agent, first, second) {
        const input = new Pushable();
        async function* messageGenerator() {
            const iter = input[Symbol.asyncIterator]();
    ...
    it("still forwards the result text after a turn failed without a result", async () => {
        ...
        injectSessionTwoTurns(agent, [messageStart("msg-a"), textDelta("partial answ"), idle], [replayedResult("**3**"), idle]);
        await expect(agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "1+2" }] })).rejects.toThrow();
        await agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "1+2" }] });
    ...
    it("forwards the result text when only subagent content preceded it", async () => {
        ...
        injectSession(agent, [
            assistantMessage("msg-sub", [{ type: "image", source: { type: "base64", data: "aGk=", media_type: "image/png" } }], "tool-1"),
            replayedResult("**3**"),
            idle,
        ]);
        await agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "1+2" }] });
...
describe("post-error recovery", () => {
    ...
    it("does not seed an orphan for a queued command whose terminal frame already passed", async () => {
        ...
        await waitFor(() => agent.sessions["test-session"]?.turnQueue?.some((t) => t.commandFinished === "completed") === true);
        await agent.cancel({ sessionId: "test-session" });
    ...
    it("does not zombify a command folded into the active turn whose shared result was the active turn's", async () => {
        ...
        await waitFor(() => agent.sessions["test-session"]?.turnQueue?.some((t) => t.commandStarted) === true);
        await agent.cancel({ sessionId: "test-session" });
        expect(agent.sessions["test-session"]?.orphanCommands?.size).toBe(1);
        const compact = agent.prompt({
            sessionId: "test-session",
    ...
    it("does not seed a zombie for a fold-aborted command whose shared result passed BEFORE the cancel", async () => {
        ...
        // commandResultSeen.
        await waitFor(() => agent.sessions["test-session"]?.turnQueue?.some((t) => t.commandFinished === "cancelled" && t.commandResultSeen === true) === true);
        await agent.cancel({ sessionId: "test-session" });
        expect(agent.sessions["test-session"]?.orphanCommands?.size ?? 0).toBe(0);
        const compact = agent.prompt({
            sessionId: "test-session",
    ...
    it("does not seed an orphan on force-cancel when the wedged turn's result and terminal frame already passed", async () => {
        ...
        const gate = new Promise((resolve) => (release = resolve));
        let releaseWedge;
        const wedge = new Promise((resolve) => (releaseWedge = resolve));
        const abortedResult = createResultMessage({
            subtype: "error_during_execution",
            stop_reason: null,
            is_error: true,
...
describe("deferred settlement for live background subagents (issues #864/#866)", () => {
    ...
    const replayedResult = (text) => resultMessage({
        result: text,
        usage: {
            input_tokens: 10,
            output_tokens: 0,
    ...
    const backgroundTasksChanged = (taskIds) => ({
        type: "system",
    ...
    it("defers a refusal result while the turn's subagent is live", async () => {
        ...
        await waitFor(() => agent.sessions["test-session"]?.activeTurn?.deferredSettle?.stopReason === "refusal");
        releaseDrain();
    ...
    it("re-arms the hold when a later level includes a previously absent task, and sweeps ended entries at activation", async () => {
        ...
        expect(agent.sessions["test-session"].liveBackgroundTasks.get("agent-1")?.endedPerLevel).toBe("sweep-armed");
    ...
    it("closes the stretch after autonomous prose so a replayed prompt still delivers", async () => {
        ...
        await waitFor(() => events.includes("chunk:background note") &&
...
describe("turn steering (_session/steering)", () => {
    ...
    function timelineClient(timeline) {
        return {
            sessionUpdate: async (notification) => {
    ...
    it("submits set and clear through the session prompt queue when the session is idle", async () => {
        ...
        await expect(agent.goal({ sessionId: "test-session", action: "set", objective: "Replacement" })).resolves.toEqual({});
    ...
    it("injects clear into a running goal at priority 'now' and publishes removal", async () => {
        ...
        expect(goalUpdates.slice(clearIndex + 1)).not.toContainEqual(expect.objectContaining({ objective: "Keep working" }));
    });
    ...
    it("does not let a stale runtime update overwrite an optimistic goal replacement", async () => {
        const updates = [];
        let staleUpdateEmitted;
        const staleUpdate = new Promise((resolve) => (staleUpdateEmitted = resolve));
        ...
        expect(goalUpdates.slice(replacementIndex + 1)).not.toContainEqual(expect.objectContaining({ objective: "Original" }));
        releaseReplacement();
        await expect(runningGoal).resolves.toEqual(expect.objectContaining({ stopReason: "end_turn" }));
        expect(goalUpdates).not.toContainEqual(expect.objectContaining({ objective: "Replacement", iterations: 1 }));
        await vi.waitFor(() => {
            expect(updates).toContainEqual({
                sessionId: "test-session",
                update: {
    ...
    it("cancels the running turn without clearing the persistent goal", async () => {
        ...
        await agent.cancel({ sessionId: "test-session" });
        releaseAfterCancel();
        await expect(runningGoal).resolves.toEqual(expect.objectContaining({ stopReason: "cancelled" }));
        expect(updates.some(({ update }) => update.sessionUpdate === "session_info_update" && update._meta?.goal === null)).toBe(false);
    });
    it("validates goal control requests", () => {
        ...
        expect(parseGoalRequest({ sessionId: "test-session", action: "set", objective: "  Replacement  " })).toEqual({ sessionId: "test-session", action: "set", objective: "Replacement" });
        expect(() => parseGoalRequest({ sessionId: "test-session", action: "set" })).toThrow('goal action "set" requires a non-empty objective');
        expect(() => parseGoalRequest({ sessionId: "test-session", action: "set", objective: "   " })).toThrow('goal action "set" requires a non-empty objective');
        expect(() => parseGoalRequest({ sessionId: "test-session", action: "pause" })).toThrow('goal action must be "set" or "clear"');
        expect(() => parseGoalRequest({ sessionId: "test-session", action: "resume" })).toThrow('goal action must be "set" or "clear"');
    ...
    it("keeps the turn open across the interrupt its own steer caused", async () => {
        ...
        await waitFor(() => !!agent.sessions["test-session"]?.activeTurn);
        await expect(agent.steer({ sessionId: "test-session", prompt: [{ type: "text", text: "also handle X" }] })).resolves.toEqual({ outcome: "injected" });
    ...
    it("cancels a steered turn without false-failing the next prompt", async () => {
        ...
        await expect(agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "next" }] })).resolves.toEqual(expect.objectContaining({ stopReason: "end_turn" }));
        expect(agent.sessions["test-session"].owedTrailingIdles).toBe(0);
    });
...
describe("turn abandoned by the SDK (issue #825)", () => {
    ...
    it("fails the in-flight prompt when the SDK goes idle without emitting a result", async () => {
        ...
        await expect(agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "first" }] })).rejects.toThrow(/without a result/);
    ...
    it("does not leak the owed idle when a cancel lands between a result and its trailer", async () => {
        ...
        await agent.cancel({ sessionId: "test-session" });
        releaseAfterCancel();
        await expect(agent.prompt({ sessionId: "test-session", prompt: [{ type: "text", text: "second" }] })).rejects.toThrow(/without a result/);
    });
});
describe("streamEventToAcpNotifications", () => {
    it("refines a tool call as soon as a streamed input field is complete", () => {
        ...
        }, "test-session", toolUseCache, {}, console, options);
    ...
    describe("partial tool input coverage", () => {
        function refineFromPartialInput({ name, partialJson, type = "tool_use", }) {
            const toolUseCache = {};
            const emittedToolCalls = new Set();
            const streamedToolInputs = new Map();
            const options = {
        ...
        it.each([
            ...
            },
            {
                ...
                name: "Grep",
                partialJson: '{"pattern":"TODO, FIXME","-i":true,"-n":true,"-A":2,"-B":1,"-C":3,"output_mode":"files_with_matches","head_limit":10,"glob":"*.ts","type":"ts","multiline":true,"path":',
                title: 'grep -i -n -A 2 -B 1 -C 3 -l | head -10 --include="*.ts" --type=ts -P "TODO, FIXME"',
                rawInput: {
                    pattern: "TODO, FIXME",
                    "-i": true,
                    "-n": true,
            ...
            },
            {
                ...
                name: "ReportFindings",
                partialJson: '{"findings":[{"file":"src/a.ts","line":7,"summary":"Broken","failure_scenario":"Fails"}],"level":',
        ...
        it.each([
            ...
            },
            {
                ...
                partialJson: '{"questions":[{"question":"Which mode?","header":"Mode","options":[{"label":"Fast","description":"Fast mode"},{"label":"Safe","description":"Safe mode"}],"multiSelect":false}]',
            },
        ])("waits for the consolidated message when $case is the only field", ({ name, partialJson }) => {
        ...
        it("keeps streamed TodoWrite input out of the tool feed", () => {
            ...
            const { started, refined } = refineFromPartialInput({
                name: "TodoWrite",
                partialJson: '{"todos":[{"content":"Run tests","status":"in_progress","activeForm":"Running tests"}]',
            });
        ...
        it("survives a ping keep-alive arriving mid-stream", () => {
            const toolUseCache = {};
            const streamedToolInputs = new Map();
            const options = { emittedToolCalls: new Set(), streamedToolInputs };
            const baseMessage = {
                type: "stream_event",
                parent_tool_use_id: null,
                uuid: randomUUID(),
            ...
            const send = (event) => streamEventToAcpNotifications({ ...baseMessage, event }, "test-session", toolUseCache, {}, console, options);
            send({
                type: "content_block_start",
                index: 0,
...
describe("tool_progress heartbeats", () => {
    ...
    async function run(messages, inFlight) {
        const updates = [];
        const mockClient = {
            sessionUpdate: async (n) => {
                updates.push(n);
    ...
    function beat(toolUseId, parentToolUseId, extra = {}) {
        return {
            type: "tool_progress",
            tool_use_id: toolUseId,
            tool_name: "Bash",
    ...
    it("reports every beat against the tool call it describes", async () => {
        const sent = await run([beat("toolu_abc-heartbeat-0", "toolu_abc"), beat("toolu_abc-heartbeat-1", "toolu_abc")], ["toolu_abc"]);
    ...
    it("preserves the elapsed time", async () => {
        const sent = await run([beat("toolu_abc-heartbeat-2", "toolu_abc", { elapsed_time_seconds: 90 })], ["toolu_abc"]);
        expect(sent[0].toolCallId).toBe("toolu_abc");
        expect(sent[0]._meta.claudeCode).toMatchObject({
            toolName: "Bash",
...
describe("permission_denied", () => {
    ...
    function toolUse(toolUseId, parentToolUseId = null) {
        return {
            type: "assistant",
            message: {
                role: "assistant",
    ...
    function toolResult(toolUseId, parentToolUseId = null) {
        return {
            type: "user",
            message: {
                role: "user",
    ...
    function denial(toolUseId, extra = {}) {
        return {
            type: "system",
    ...
    const denials = (updates) => updates.filter((u) => u.sessionUpdate === "tool_call_update" &&
    ...
    it("attributes a subagent denial to the Agent call that spawned it", async () => {
        ...
        expect(denials(updates)[0]._meta.claudeCode).toMatchObject({
            toolName: "Write",
