...
import { buildConfigOptions, clientSupportsBooleanConfigOptions, createFastModeConfigOption, fastModeStateEnabled, normalizeFastModeDisabledReason, resolveFastModeEnabled, FAST_MODE_CONFIG_ID, FAST_MODE_ON, FAST_MODE_OFF, ClaudeAcpAgent, } from "../acp-agent.js";
const MODES = {
...
describe("createFastModeConfigOption", () => {
    ...
    it("explains a blocking disabled reason while the toggle reads off", () => {
        ...
        expect(option.description).toBe("Faster responses on supported models — not available on the free plan");
    });
    it("ignores routine and on-state reasons", () => {
        ...
        expect(createFastModeConfigOption(false, true, "sdk_opt_in_required").description).toBe("Faster responses on supported models");
        ...
        expect(createFastModeConfigOption(true, true, "free").description).toBe("Faster responses on supported models");
    });
...
describe("clientSupportsBooleanConfigOptions", () => {
    it("is true only when session.configOptions.boolean is present", () => {
        expect(clientSupportsBooleanConfigOptions({ session: { configOptions: { boolean: {} } } })).toBe(true);
    });
    it("is false when the capability is omitted or null at any level", () => {
        ...
        expect(clientSupportsBooleanConfigOptions({ session: { configOptions: { boolean: null } } })).toBe(false);
    });
});
describe("resolveFastModeEnabled", () => {
    ...
    it("rejects any other value", () => {
        expect(() => resolveFastModeEnabled({ ...base, value: "maybe" })).toThrow(/Invalid value for config option fast/);
    });
});
...
describe("normalizeFastModeDisabledReason", () => {
    ...
    it("drops routine states and unknown values", () => {
        ...
        expect(normalizeFastModeDisabledReason("some_future_reason")).toBeUndefined();
    });
});
...
describe("setSessionConfigOption Fast mode toggle", () => {
    ...
    function setup(opts) {
        ...
        const clientCapabilities = opts.useBooleanOption
    ...
    it("toggles Fast mode on/off through a boolean value", async () => {
        ...
        expect(agent.sessions[SESSION_ID].fastModeEnabled).toBe(true);
...
describe("syncFastModeState (SDK-driven state changes)", () => {
    ...
    function setup(opts) {
        ...
        const sync = agent.syncFastModeState.bind(agent);
    ...
    it("emits a config_option_update when the SDK reports a new state", async () => {
        ...
        const updated = sessionUpdates[0].update.configOptions;
    ...
    it("explains a blocking reason once when the SDK refuses a toggle the user turned on", async () => {
        ...
        expect(session.configOptions).toContainEqual(createFastModeConfigOption(false, true, "extra_usage_disabled"));
    ...
    it("updates the description when only the reason changes", async () => {
        ...
        expect(session.configOptions).toContainEqual(createFastModeConfigOption(false, true, "not_first_party"));
    });
